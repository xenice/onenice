    ...
    <div class="row">
      <div class="user-left col-md-2">
          ...
          <?php
            $arr = [
                'profile' => _t('Profile'),
                'security'=> _t('Security')
                ];
            foreach($arr as $key=>$value){
                if($key == $action){
                    echo '<li class="nav-item current-menu-item"><a class="nav-link" href="'.$page->url().'?action='.$key.'">'.$value.'</a></li>';
                }
                else{
                    echo '<li class="nav-item"><a class="nav-link" href="'.$page->url().'?action='.$key.'">'.$value.'</a></li>';
                }
            }
          ?>
        </ul>
...
$template->js .=<<<js
jQuery(function($){
    xenice.collapse('.arrow','.user-left','.user-right','.user-shade');
